...
import { AlertTriangle, CheckCircle, Clock, TrendingUp, Calendar, Users, FileText, Bell, Plus } from 'lucide-react';
...
function StatsCard({ title, value, icon: Icon, color, bgColor, description, isLoading }: StatsCardProps) {
  return (
...
export function Dashboard({ onNewTask, onViewChange }: DashboardProps) {
  const { allTasks, getTaskStats, isLoading, error } = useTasks();
  const { user } = useAuth();
